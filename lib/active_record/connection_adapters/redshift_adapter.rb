      ...
      private
        ...
        def column_definitions(table_name) # :nodoc:
          ...
          SELECT column_name as attname, data_type as format_type, '' as pg_get_expr,
                 not (is_nullable = 't') as attnotnull, data_type as atttypid, -1 as atttypmod
            FROM svv_redshift_columns
           WHERE table_name = '#{quote_table_name(table_name)}'
             AND schema_name = 'etlschema'
          end_sql
          #The below lines doesnt support external columns
          #For AWS Data share we need external columns to be mapped with postgres
          # exec_query(<<-end_sql, 'SCHEMA').rows
